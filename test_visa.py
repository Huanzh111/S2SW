...
from visa_wavelet_transform import apply_wavelet_transform
...
def test(args):
    ...
    for idx, items in enumerate(tqdm(test_dataloader)):
        ...
        with torch.no_grad():
            ...
            # text_features = (text_features_yuan + text_features_emo_yuan) / 2.0

            anomaly_map_list = []
            # similarity_fre_map_list = []
            for idx, patch_feature in enumerate(patch_features):
                if idx >= args.feature_map_layer[0]:
                    ...
                    if args.metrics == 'pixel-level':
                        ...
                        patch_feature_fus = multi_scale(patch_feature) * 0.2 + patch_feature * 0.8
                        patch_feature_fus = patch_feature_fus / patch_feature_fus.norm(dim=-1, keepdim=True)
                        frequency_features = patch_feature
                    similarity_fre, _ = AnomalyCLIP_lib.compute_similarity(frequency_features, text_features_fustion[0], text_features_fustion[0])
                    ...
                    similarity, _ = AnomalyCLIP_lib.compute_similarity(patch_feature_fus, text_features_fustion[0], text_features_fustion[0])
                    similarity_map = AnomalyCLIP_lib.get_similarity_map(similarity[:, 1:, :], args.image_size)
                    anomaly_map = (similarity_map[..., 1] + 1 - similarity_map[..., 0]) / 2.0
                    anomaly_map = anomaly_map * 0.5 + anomaly_map_fre * 0.5
...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_path", type=str, default='./results_ablation/', help='path to save results')
    parser.add_argument("--checkpoint_path", type=str, default='./visaResults_liu/epoch_6.pth', help='path to checkpoint')
    # model
